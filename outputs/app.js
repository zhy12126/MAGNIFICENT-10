const fallbackStocks=[
 ...
 {name:'Sandisk',ticker:'SNDK',logo:'S',color:'#fff0ea',ink:'#d04d34',cap:'—',pe:'—',fpe:'—',peg:'—',ps:'—',pcf:'—',implied:'—',growth:'—',price:'等待日终更新',note:'等待 Alpha Vantage 日更数据。'}
];
...
const renderShareMetric=(key,valueId,noteId,marketValueNoteId)=>{const metric=concentrationData?.metrics?.[key],value=document.querySelector(valueId),note=document.querySelector(noteId),marketValueNote=document.querySelector(marketValueNoteId),hasShare=metric?.share!==null&&metric?.share!==undefined&&metric?.share!==''&&Number.isFinite(Number(metric.share));if(value)value.textContent=hasShare?`${Number(metric.share).toFixed(2)}%`:'—';if(note){const hasDelta=metric?.dailyChangePp!==null&&metric?.dailyChangePp!==undefined&&Number.isFinite(Number(metric.dailyChangePp)),delta=Number(metric?.dailyChangePp);if(hasDelta){note.textContent=`权重较前日 ${delta>0?'+':''}${delta.toFixed(2)}pp`;note.className=delta>0?'positive':delta<0?'negative':''}else{note.textContent=hasShare?'首个快照，等待下一交易日':'等待 SPY 日持仓更新';note.className=''}}if(marketValueNote){const rawChange=metric?.dailyBasketUnitValueChangePct,hasBasketUnitValueChange=rawChange!==null&&rawChange!==undefined&&rawChange!==''&&Number.isFinite(Number(rawChange)),change=Number(rawChange);if(hasBasketUnitValueChange){marketValueNote.textContent=`SPY 每份额篮子价值较前日 ${change>0?'+':''}${change.toFixed(2)}%`;marketValueNote.className=change>0?'positive':change<0?'negative':''}else{marketValueNote.textContent=concentrationData?.spyUnitPrice?'SPY 每份额篮子价值：等待下一交易日':'SPY 每份额篮子价值：等待数据';marketValueNote.className=''}}};
const renderMag7Share=()=>{renderShareMetric('mag7','#mag7-share','#mag7-share-note','#mag7-market-value-note');renderShareMetric('aiHardware','#semi-share','#semi-share-note','#semi-market-value-note')};
const brandDomains={NVDA:'nvidia.com',AAPL:'apple.com',MSFT:'microsoft.com',GOOGL:'google.com',AMZN:'amazon.com',META:'meta.com',TSLA:'tesla.com',TSM:'tsmc.com',MU:'micron.com',AVGO:'broadcom.com',AMD:'amd.com',SNDK:'sandisk.com'};
...
function modelInputs(s){const m=s.valuationModel,hasResult=s.implied&&s.implied!=='—'&&m?.status==='ready';if(!m||m.status!=='ready')return `<p class="eyebrow">IMPLIED-GROWTH INPUTS</p><h3>公司级模型输入</h3><p class="model-result-unavailable">未展示推算结果：当前没有足够的公司公开财报数据，无法建立可靠的公司级现金流模型，因此隐含增长率显示为“—”。</p>`;const pct=v=>`${(Number(v)*100).toFixed(1)}%`,reason=m.impliedGrowthNote||'当前现金流数据不适合得出可靠的长期增长率。';const comparison=hasResult?`<div class="growth-compare" aria-label="增长对比"><div><span>未来 5 年所需增长</span><b>${s.implied}</b><small>模型反推的收入 CAGR</small></div><i aria-hidden="true">对比</i><div><span>最近季度实际增长</span><b>${s.growth||'—'}</b><small>已披露收入同比</small></div></div>`:'';const disclosure=hasResult?'':`<p class="model-result-unavailable">未展示推算结果：${reason}</p>`;return `<div class="model-head"><div><p class="eyebrow">IMPLIED-GROWTH INPUTS</p><h3>公司级模型输入</h3></div>${comparison}</div><div class="model-grid"><span>TTM 自由现金流率<b>${pct(m.fcfMarginTTM)}</b></span><span>三年中位数<b>${pct(m.fcfMargin3yMedian)}</b></span><span>归一化 FCF 率<b>${pct(m.normalizedFcfMargin)}</b></span><span>权益成本<b>${pct(m.costOfEquity)}</b></span><span>Beta<b>${Number(m.beta).toFixed(2)}</b></span><span>永续增长<b>${pct(m.terminalGrowth)}</b></span></div>${disclosure}<p class="model-formula-note">未来 5 年所需增长 g：求解 <b>当前市值 = Σ（第 t 年收入 × 自由现金流率 ÷ (1 + 权益成本)<sup>t</sup>）+ 终值折现</b>，其中第 t 年收入 = 当前收入 × (1 + g)<sup>t</sup>。</p><p class="model-note">截止财报期：${m.fiscalPeriodEnd}。${m.rationale} 数据来源：${m.source}。</p>`}
