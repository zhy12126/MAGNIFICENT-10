...
function drawChart(s) {
  ...
  const priceNote = showPriceOverlay ? points.some(point => point.priceKind === 'underlying-ads-equivalent-proxy') ? '，蓝线为股价；橙色虚线为上市前 ADS 代理价。' : '，蓝线为股价（右轴）' : '';
  ...
  const overlayLine = prices ? `${pricePaths.actual ? `<path class="line price-overlay-line price-overlay-actual" stroke="var(--blue)" stroke-width="2.25" d="${pricePaths.actual}"/>` : ''}${pricePaths.proxy ? `<path class="line price-overlay-line price-overlay-proxy" stroke="#f0a500" stroke-width="2.25" d="${pricePaths.proxy}"/>` : ''}<text class="axis price-axis price-axis-title" x="1028" y="48" text-anchor="end">股价</text>` : '';
