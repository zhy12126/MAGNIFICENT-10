const fallbackStocks = [
  ...
  ,{ name: 'Netflix', ticker: 'NFLX', logo: 'N', color: '#fff0f0', ink: '#e50914', cap: '—', pe: '—', fpe: '—', peg: '—', ps: '—', pcf: '—', implied: '—', growth: '—', price: '等待日终更新', note: '等待数据更新。' }
  ,{ name: "McDonald's", ticker: 'MCD', logo: 'M', color: '#fff6d8', ink: '#da291c', cap: '—', pe: '—', fpe: '—', peg: '—', ps: '—', pcf: '—', implied: '—', growth: '—', price: '等待日终更新', note: '等待数据更新。' }
  ,{ name: 'Palantir', ticker: 'PLTR', logo: 'P', color: '#eef0f2', ink: '#101113', cap: '—', pe: '—', fpe: '—', peg: '—', ps: '—', pcf: '—', implied: '—', growth: '—', price: '等待日终更新', note: '等待数据更新。' }
  ,{ name: 'Eli Lilly', ticker: 'LLY', logo: 'L', color: '#fff0f1', ink: '#d52b1e', cap: '—', pe: '—', fpe: '—', peg: '—', ps: '—', pcf: '—', implied: '—', growth: '—', price: '等待日终更新', note: '等待数据更新。' }
  ,{ name: 'Oracle', ticker: 'ORCL', logo: 'O', color: '#fff0f0', ink: '#c74634', cap: '—', pe: '—', fpe: '—', peg: '—', ps: '—', pcf: '—', implied: '—', growth: '—', price: '等待日终更新', note: '等待数据更新。' }
  ,{ name: 'American Express', ticker: 'AXP', logo: 'A', color: '#e9f5ff', ink: '#006fcf', cap: '—', pe: '—', fpe: '—', peg: '—', ps: '—', pcf: '—', implied: '—', growth: '—', price: '等待日终更新', note: '等待数据更新。' }
];
...
const brandDomains = { NVDA: 'nvidia.com', AAPL: 'apple.com', MSFT: 'microsoft.com', GOOGL: 'google.com', AMZN: 'amazon.com', META: 'meta.com', TSLA: 'tesla.com', TSM: 'tsmc.com', MU: 'micron.com', AVGO: 'broadcom.com', AMD: 'amd.com', SKHY: 'skhynix.com', NFLX: 'netflix.com', MCD: 'mcdonalds.com', PLTR: 'palantir.com', LLY: 'lilly.com', ORCL: 'oracle.com', AXP: 'americanexpress.com' };
...
const CAPEX_SCENARIO_TICKERS = new Set(['MSFT', 'GOOGL', 'META', 'AMZN', 'ORCL']);
const CYCLE_SCENARIO_TICKERS = new Set(['TSLA', 'TSM', 'MU', 'SKHY']);
const SPECIAL_VALUATION_SCENARIOS = {
  ORCL: { title: 'AI 云合同与资本回收情景', note: '当前负 FCF 主要反映数据中心建设期，不机械永久化；模型以十年合同兑现期逐步恢复现金转化，并保留净债务扣减。', currentMargin: .02, scenarios: [['建设期延长', .16, .10], ['合同逐步兑现', .25, .15], ['云规模回报', .30, .20]] },
  PLTR: { title: '扣除股权激励后的十年所有者收益', note: '经营现金流中的股权激励加回会高估股东真实现金流；延长预测期，同时继续扣除稀释成本。', currentMargin: .35, scenarios: [['高稀释', .28, .30], ['基准稀释', .35, .40], ['规模效应', .42, .50]] },
  LLY: { title: '药品组合与管线风险调整情景', note: '用十年显性期覆盖GLP-1放量、后续管线、竞争与专利周期；增长逐年降速，不把单一产品永久化。', scenarios: [['专利压力', .14, .10], ['组合基准', .18, .18], ['管线成功', .22, .25]] },
  TSLA: { title: '汽车、储能与软件期权情景', note: '十年显性期分别表达汽车制造、储能扩张和软件兑现；结果不把尚未商业化的机器人收入直接计入基准。', scenarios: [['汽车主导', .05, .05], ['汽车+能源', .10, .15], ['软件期权兑现', .16, .30]] },
  AMZN: { title: 'AWS、广告与零售现金回报情景', note: '当前 FCF 受AI数据中心和物流投入压低；十年期让AWS、广告与零售效率逐步反映到所有者收益。', scenarios: [['高投入延续', .07, .08], ['混合基准', .12, .12], ['规模回报', .16, .16]] },
  NVDA: { title: 'AI 平台十年周期压力测试', note: '十年显性期覆盖AI基础设施扩张与增速回落；估值权益成本采用11%，原始高波动Beta仍在参数区展示。', valuationCostOfEquity: .11, scenarios: [['周期回落', .35, .25], ['结构基准', .46, .35], ['强景气', .50, .45]] },
  AMD: { title: '数据中心、客户端与AI十年压力测试', note: '延长到十年覆盖份额提升和半导体周期；成熟现金流率不再停留在偏低的10%。', scenarios: [['下行周期', .12, .15], ['完整周期', .20, .25], ['AI 放量', .26, .35]] },
};
const REFERENCE_GROWTH = { AAPL:.08,MSFT:.16,GOOGL:.16,META:.16,TSM:.15,MU:.15,AVGO:.15,SKHY:.10,NFLX:.12,MCD:.05 };
const COMPANY_FAR_GROWTH = {
  NVDA:[.18,.14],AAPL:[.06,.05],MSFT:[.14,.12],GOOGL:[.12,.10],AMZN:[.12,.10],META:[.12,.10],TSLA:[.14,.11],TSM:[.13,.10],MU:[.07,.05],AVGO:[.15,.12],AMD:[.17,.13],SKHY:[.08,.06],NFLX:[.10,.08],MCD:[.05,.04],PLTR:[.23,.19],LLY:[.18,.14],ORCL:[.17,.13]
};
const COMPANY_MODEL_PROFILES = {
  NVDA:{ tail:[.10,.08,.07,.06,.05], years:10, valuationCost:.11, note:'十年AI基础设施与平台护城河模型' },
  AAPL:{ tail:[.05,.05,.045,.04,.035], years:10, targetMargin:.29, valuationCost:.085, note:'十年每股所有者收益模型；回购仅通过每股现金创造体现，不机械加成' },
  MSFT:{ tail:[.10,.09,.08,.07,.06], years:10, targetMargin:.30, valuationCost:.09, note:'Azure、Microsoft 365与AI资本回收十年模型' },
  GOOGL:{ tail:[.09,.08,.07,.06,.05], years:10, targetMargin:.26, valuationCost:.095, note:'广告、Cloud与AI基础设施现金回报十年模型' },
  AMZN:{ tail:[.10,.09,.08,.07,.06], years:10, valuationCost:.10, note:'AWS、广告与零售混合十年模型' },
  TSLA:{ tail:[.10,.09,.08,.07,.06], years:10, valuationCost:.11, note:'汽车、储能与软件期权十年情景' },
  TSM:{ tail:[.09,.08,.07,.06,.05], years:10, targetMargin:.28, valuationCost:.10, note:'先进制程与晶圆厂完整投资周期十年模型' },
  MU:{ tail:[.05,.04,.04,.03,.03], years:10, targetMargin:.18, valuationCost:.12, note:'存储周期中位现金流十年模型' },
  AVGO:{ tail:[.10,.09,.08,.07,.06], years:10, targetMargin:.43, valuationCost:.105, note:'AI半导体、网络与基础设施软件十年模型' },
  AMD:{ tail:[.12,.10,.08,.07,.05], years:10, valuationCost:.12, note:'数据中心、客户端与AI份额十年模型' },
  SKHY:{ tail:[.05,.04,.04,.03,.03], years:10, targetMargin:.20, valuationCost:.12, note:'HBM与传统存储跨周期十年模型' },
  NFLX:{ tail:[.08,.07,.06,.05,.04], years:10, targetMargin:.25, valuationCost:.10, note:'订阅、广告与内容现金回收十年模型' },
  PLTR:{ tail:[.17,.15,.13,.11,.09], years:10, valuationCost:.105, note:'AIP扩张与股权稀释十年模型' },
  LLY:{ tail:[.12,.10,.08,.06,.04], years:10, valuationCost:.085, note:'药品组合与管线风险调整十年模型' },
  ORCL:{ tail:[.12,.10,.08,.07,.06], years:10, valuationCost:.10, note:'云合同兑现与资本回收十年模型' },
};
const COMPANY_REVENUE_GUIDANCE = {
  NVDA:{ value:'91%–99%', period:'FY2027 Q2收入 $89.2B–$92.8B，对比 FY2026 Q2实际收入', issued:'2026-05-20' },
  AAPL:{ value:'9%–11%', period:'FY2026 Q4（截至9月）总收入，对比 FY2025 Q4实际收入；iPhone约增长15%', issued:'2026-07-30' },
  MSFT:{ value:'16%–17%', period:'FY2027 Q1收入 $89.85B–$90.95B，对比 FY2026 Q1实际收入 $77.67B', issued:'2026-07-29' },
  GOOGL:{ value:'未量化', period:'2026 Q3定性展望，对比2025 Q3：Cloud保持强劲增长；Search进入较高基数期，汇率约由Q2顺风1个百分点转为轻微逆风', issued:'2026-07-22' },
  AMZN:{ value:'9%–12%', period:'2026 Q3净销售额 $197B–$202B，对比2025 Q3实际净销售额', issued:'2026-07-30' },
  TSM:{ value:'35%–38%', period:'2026 Q3美元收入 $44.6B–$45.8B，对比2025 Q3实际美元收入', issued:'2026-07-16' },
  MU:{ value:'333%–351%', period:'FY2026 Q4收入 $49.0B–$51.0B，对比 FY2025 Q4实际收入', issued:'2026-06-24' },
  AVGO:{ value:'约 84%', period:'FY2026 Q3收入约 $29.4B，对比 FY2025 Q3实际收入', issued:'2026-06-03' },
  AMD:{ value:'约 41%', period:'2026 Q3收入约 $13.0B（±$0.3B），对比2025 Q3实际收入 $9.246B', issued:'2026-08-04' },
  PLTR:{ value:'约 82%', period:'FY2026全年收入 $8.150B–$8.158B，对比 FY2025全年实际收入', issued:'2026-08-03' },
  LLY:{ value:'30%–33%', period:'2026全年收入 $85B–$87B，对比2025全年实际收入 $65.2B', issued:'2026-08-05' },
  ORCL:{ value:'约 34%', period:'FY2027全年收入约 $90B，对比 FY2026全年实际收入', issued:'2026-06-10' },
};
const ANALYST_REVENUE_EXPECTATIONS = {
  AAPL:{ value:'约 13.0%', period:'FY2026 Q4收入共识 $115.73B，对比 FY2025 Q4实际 $102.47B', source:'Zacks', updated:'2026-08' },
  GOOGL:{ value:'约 24.0%', period:'2026 Q3收入共识 $126.9B，对比 2025 Q3实际约 $102.35B', source:'S&P Global Market Intelligence', updated:'2026-07' },
};
const BUSINESS_GROWTH = {
  NVDA:{ period:'FY2027 Q1 vs FY2026 Q1', issued:'2026-05-20', source:'NVIDIA 财报', mix:[['Hyperscale数据中心',37.9],['AI Cloud、工业与企业',37.4],['边缘计算',6.3]], items:[['Hyperscale 数据中心','$37.9B','+115%'],['AI Cloud、工业与企业','$37.4B','+74%'],['边缘计算','$6.3B','+29%']] },
  AAPL:{ period:'FY2026 Q3 vs FY2025 Q3', issued:'2026-07-30', source:'Apple FY2026 Q3财务报表', mix:[['iPhone',54.25],['Mac',10.35],['iPad',6.19],['可穿戴等',7.89],['服务',30.74]], items:[['iPhone','$54.25B','+21.7%'],['Mac','$10.35B','+28.6%'],['iPad','$6.19B','−5.9%'],['可穿戴、家居与配件','$7.89B','+6.6%'],['服务','$30.74B','+12.1%']] },
  MSFT:{ period:'微软FY2026 Q4（自然年2026 Q2）vs 微软FY2025 Q4', issued:'2026-07-29', source:'Microsoft FY2026 Q4财报（截至2026-06-30）', mix:[['生产力与业务流程',37.8],['云业务（Intelligent Cloud）',38.6],['更多个人计算',13.6]], items:[['生产力与业务流程','$37.8B','+14%'],['云业务（Intelligent Cloud）','约$38.6B','约+29%'],['更多个人计算','约$13.6B','约+1%'],['其中：Azure及其他云服务','公司未单列金额','+43%']] },
  GOOGL:{ period:'2026 Q2 vs 2025 Q2', issued:'2026-07-22', source:'Alphabet 财报', mix:[['Google Services',94.5],['Google Cloud',24.77],['Other Bets及其他',.53]], items:[['Google Search及其他','$63.27B','+17%'],['YouTube广告','$11.06B','+13%'],['订阅、平台与设备','公司未单列金额','+15%'],['Google Cloud','$24.77B','+82%']] },
  AMZN:{ period:'2026 Q2 vs 2025 Q2', issued:'2026-07-30', source:'Amazon 财报', mix:[['北美',116.2],['国际',42.2],['AWS',42.23]], items:[['北美业务','$116.2B','+16%'],['国际业务','$42.2B','+15%'],['AWS','$42.23B','+37%'],['广告服务','公司未单列金额','+26%']] },
  META:{ period:'2026 Q2 vs 2025 Q2', issued:'2026-07-29', source:'Meta 财报', mix:[['Family of Apps',60.37],['Reality Labs',.431]], items:[['广告','$59.36B','+27%'],['Family of Apps','$60.37B','+28%'],['Reality Labs','$0.43B','+16%'],['广告展示量','非收入指标','+14%']] },
  TSLA:{ period:'2026 Q2 vs 2025 Q2', issued:'2026-07-22', source:'Tesla 财报', mix:[['汽车业务',20.5],['能源发电与储能',3.1],['服务及其他',4.6]], items:[['汽车业务','$20.5B','+23%'],['能源发电与储能','$3.1B','+13%'],['服务及其他','$4.6B','约+100%']] },
  TSM:{ period:'2026 Q2 vs 2025 Q2', issued:'2026-07-16', source:'TSMC 财报', mix:[['高性能计算',26.532],['智能手机',8.844],['其他平台',4.824]], items:[['高性能计算','占晶圆收入66%','公司未披露同比'],['智能手机','占晶圆收入22%','公司未披露同比'],['汽车电子','公司披露占比','公司未披露同比'],['总收入','$40.2B','+36%']] },
  MU:{ period:'FY2026 Q3 vs FY2025 Q3', issued:'2026-06-24', source:'Micron 财报', mix:[['DRAM',31.3],['NAND',9.9],['其他',.26]], items:[['DRAM','$31.3B','+343%'],['NAND','$9.9B','+361%']] },
  AVGO:{ period:'FY2026 Q2 vs FY2025 Q2', issued:'2026-06-03', source:'Broadcom 财报', mix:[['半导体解决方案',15.01],['基础设施软件',7.18]], items:[['半导体解决方案','$15.01B','+79%'],['AI半导体','$10.8B','+143%'],['基础设施软件','$7.18B','+9%']] },
};
const BUSINESS_GROWTH_AUDITED = {
  NVDA:{period:'FY2027 Q1 vs FY2026 Q1',issued:'2026-05-20',source:'NVIDIA FY2027 Q1财报及FY2026市场平台收入表',segments:[['AI数据中心',75.246,'+92%','AI训练、推理、加速计算及数据中心网络；本季收入占比92.2%'],['玩家、终端与边缘计算',6.369,'+29%','GeForce游戏、PC与游戏主机、工作站、汽车及机器人；公司本季未再单列纯Gaming']],submetrics:[['Hyperscale AI客户','+115%','AI数据中心子业务',50.4],['AI云、工业与企业','+74%','AI数据中心子业务',49.7],['Gaming（最近单列完整财年）','+41%','FY2026收入$16.042B，占FY2026公司总收入7.4%；对比FY2025']]},
  AAPL:{period:'FY2026 Q3 vs FY2025 Q3',issued:'2026-07-30',source:'Apple FY2026 Q3财务报表',segments:[['iPhone',54.25,'+21.7%','iPhone手机及相关硬件'],['Mac',10.35,'+28.6%','Mac电脑'],['iPad',6.19,'−5.9%','iPad平板电脑'],['可穿戴、家居与配件',7.89,'+6.6%','Watch、AirPods、Home及配件'],['服务',30.74,'+12.1%','App Store、订阅、广告、云服务等']]},
  MSFT:{period:'微软FY2026 Q4（自然年2026 Q2）vs 微软FY2025 Q4',issued:'2026-07-29',source:'Microsoft FY2026 Q4财报',segments:[['生产力与业务流程',37.8,'+14%','Microsoft 365、LinkedIn、Dynamics'],['云业务（Intelligent Cloud）',38.6,'约+29%','Azure、服务器产品及企业服务'],['更多个人计算',13.6,'约+1%','Windows、设备、游戏和搜索广告']],submetrics:[['Azure及其他云服务','+43%','云分部中的核心子业务']]},
  GOOGL:{period:'2026 Q2 vs 2025 Q2',issued:'2026-07-22',source:'Alphabet 财报',segments:[['Google Services',94.5,'+15%','搜索、YouTube、订阅、平台与设备'],['Google Cloud',24.77,'+82%','云基础设施、平台及Workspace'],['Other Bets及其他',.53,'未披露','Waymo等早期业务及公司层调整']],submetrics:[['搜索及其他','+17%','Google Services子业务',67.0],['YouTube广告','+13%','Google Services子业务',11.7]]},
  AMZN:{period:'2026 Q2 vs 2025 Q2',issued:'2026-07-30',source:'Amazon 财报',segments:[['北美',116.2,'+16%','美国、加拿大及墨西哥零售与服务'],['国际',42.2,'+15%','北美以外零售与服务'],['AWS',42.23,'+37%','云计算、存储、数据库及AI服务']],submetrics:[['广告服务','+26%','计入北美与国际分部，不单独计入饼图']]},
  META:{period:'2026 Q2 vs 2025 Q2',issued:'2026-07-29',source:'Meta 2026 Q2财报',segments:[['广告收入',59.363,'+27%','Facebook、Instagram、Messenger、WhatsApp及Threads广告'],['应用及其他收入',1.007,'+73%','WhatsApp付费消息、Meta Verified、支付及开发者费用'],['Reality Labs',.431,'+16%','Quest、Ray-Ban Meta及AR/VR软硬件与内容']],submetrics:[['广告展示量','+14%','对比2025 Q2；广告收入增长驱动'],['平均广告价格','+12%','对比2025 Q2；广告收入增长驱动'],['Family日活人数','+3%','2026年6月平均3.60B，对比2025年6月']]},
  TSLA:{period:'2026 Q2 vs 2025 Q2',issued:'2026-07-22',source:'Tesla 财报',segments:[['汽车业务',20.5,'+23%','整车销售、租赁及监管积分'],['能源发电与储能',3.1,'+13%','Megapack、Powerwall及太阳能'],['服务及其他',4.6,'约+100%','售后、充电、保险及二手车']]},
  TSM:{period:'2026 Q2 vs 2025 Q2（美元口径）',issued:'2026-07-16',source:'TSMC 2026 Q2财报及平台收入占比',segments:[['高性能计算',26.532,'约+47.1%','AI、服务器、CPU/GPU及网络芯片'],['智能手机',8.844,'约+8.9%','手机应用处理器及通信芯片'],['物联网',2.010,'约+33.7%','IoT及边缘设备芯片'],['汽车电子',1.608,'约+7.0%','汽车计算、控制及功率相关芯片'],['数字消费电子',.402,'约+33.7%','消费电子芯片'],['其他',.804,'约+33.7%','未归入主要平台的晶圆收入']],submetrics:[['公司总收入（美元）','+33.7%','与页面顶部33%相同，顶部为四舍五入显示'],['公司总收入（新台币）','+36.0%','受汇率影响，与美元口径不同'],['平台同比算法','—','本季平台收入÷去年同期平台收入−1；平台收入=当季美元总收入×公司披露占比，结果受占比四舍五入影响']]},
  MU:{period:'FY2026 Q3 vs FY2025 Q3',issued:'2026-06-24',source:'Micron 财报',segments:[['DRAM（含HBM）',31.3,'+343%','HBM、服务器及客户端DRAM内存'],['NAND',9.9,'+361%','数据中心SSD、移动及消费存储'],['其他',.26,'未披露','未归入DRAM与NAND的产品']],submetrics:[['HBM','未单列','HBM属于DRAM；公司未持续披露独立季度收入，因此不从DRAM中重复拆分']]},
  AVGO:{period:'FY2026 Q2 vs FY2025 Q2',issued:'2026-06-03',source:'Broadcom 财报',segments:[['半导体解决方案',15.01,'+79%','AI加速器、网络、无线及存储芯片'],['基础设施软件',7.18,'+9%','VMware Cloud Foundation及企业软件']],submetrics:[['AI半导体','+143%','半导体解决方案子业务',72.0]]},
  AMD:{period:'2026 Q2 vs 2025 Q2',issued:'2026-08-04',source:'AMD 财报',segments:[['数据中心',6.718,'约+110%','EPYC服务器CPU及Instinct AI加速器'],['客户端与游戏',3.841,'约+7%','Ryzen客户端CPU、Radeon及游戏主机芯片'],['嵌入式',.977,'约+19%','工业、通信、汽车及自适应计算']],submetrics:[['客户端业务','+23%','客户端与游戏子业务',80.7],['游戏业务','−31%','客户端与游戏子业务',19.3]]},
  SKHY:{period:'2026 Q2 vs 2025 Q2（美元口径）',issued:'2026-07-29',source:'SK hynix 2026 Q2财报、产品收入占比及历史产品收入表',currency:'₩',amountUnit:'T',segments:[['DRAM（含HBM）',57.903,'约+239%','HBM、服务器及常规DRAM；本季占收入73%'],['NAND及企业SSD',21.416,'约+356%','NAND Flash与企业级SSD；本季占收入27%']],submetrics:[['公司总收入（美元）','+233%','与页面顶部233%相同；按各期美元折算金额比较'],['公司总收入（韩元）','+256.8%','₩79.319T，对比2025 Q2的₩22.232T；汇率变化导致与美元口径不同'],['产品同比算法','—','本季产品收入=Q2韩元总收入×公司披露占比；再与2025 Q2同类产品收入比较，受占比四舍五入影响'],['HBM','未单列','属于DRAM；公司未披露独立季度收入及同比']]},
  NFLX:{period:'2026 Q2 vs 2025 Q2',issued:'2026-07-16',source:'Netflix 2026 Q2 Form 10-Q地区收入表',segments:[['美国与加拿大（UCAN）',5.432,'+10%','美国和加拿大会员、广告及相关收入'],['欧洲、中东与非洲（EMEA）',4.034,'+14%','欧洲、中东与非洲地区收入'],['拉丁美洲（LATAM）',1.584,'+21%','墨西哥、中南美洲及加勒比地区收入'],['亚太地区（APAC）',1.510,'+16%','亚洲、澳大利亚及太平洋地区收入']]},
  MCD:{period:'2026 Q2 vs 2025 Q2',issued:'2026-08-04',source:"McDonald's 2026 Q2财报",segments:[['特许经营餐厅收入',4.393,'+4%','租金、特许权使用费及初始加盟费'],['直营餐厅销售',2.525,'+3%','公司直接经营餐厅的销售收入'],['其他收入',.182,'+6%','技术费、品牌授权及其他收入']]},
  PLTR:{period:'2026 Q2 vs 2025 Q2',issued:'2026-08-03',source:'Palantir 财报',segments:[['政府业务',.990,'约+72%','美国及国际政府机构合同'],['商业业务',.945,'约+122%','美国及国际企业客户']],submetrics:[['美国商业','+149%','商业业务子项'],['美国政府','+90%','政府业务子项']]},
  LLY:{period:'2026 Q2 vs 2025 Q2',issued:'2026-08-05',source:'Eli Lilly 2026 Q2财报',segments:[['Mounjaro',9.94,'+91%','糖尿病适应症的替尔泊肽'],['Zepbound',4.93,'+44%','肥胖及体重管理适应症的替尔泊肽'],['其他药品及收入',8.10,'约+16%','Verzenio、Jardiance、Trulicity、Taltz、Kisunla及其他']],submetrics:[['公司总收入','+48%','$22.97B，对比2025 Q2的$15.56B']]},
  ORCL:{period:'FY2026 Q4 vs FY2025 Q4',issued:'2026-06-10',source:'Oracle 财报',segments:[['云基础设施 IaaS',5.8,'+93%','OCI计算、存储、数据库及AI基础设施'],['云应用 SaaS',4.1,'+10%','Fusion、NetSuite及行业云应用'],['软件',6.8,'−2%','本地软件许可与支持'],['服务',1.5,'+13%','咨询与实施服务'],['硬件',.9,'+9%','Exadata及服务器硬件']]},
  AXP:{period:'2026 Q2 vs 2025 Q2',issued:'2026-07-24',source:'American Express 2026 Q2分部业绩表',segments:[['美国消费者服务',9.524,'+11%','美国个人卡、消费信贷及相关服务'],['商业服务',4.503,'+7%','中小企业与大型企业支付、融资服务'],['国际卡服务',3.619,'+12%','美国以外个人及商业卡业务'],['全球商户与网络服务',2.096,'+8%','商户收单、处理及网络合作']],submetrics:[['公司总收入（扣除利息支出）','+10%','$19.637B，对比2025 Q2的$17.856B；四个经营分部合计另有公司层抵销'],['持卡人消费额','+9%','交易规模指标，对比2025 Q2；不是收入分部']]},
};
const SUBMETRIC_PARENT={
  NVDA:{'Hyperscale AI客户':'AI数据中心','AI云、工业与企业':'AI数据中心'},
  MSFT:{'Azure及其他云服务':'云业务（Intelligent Cloud）'},
  GOOGL:{'搜索及其他':'Google Services','YouTube广告':'Google Services'},
  AVGO:{'AI半导体':'半导体解决方案'},
  AMD:{'客户端业务':'客户端与游戏','游戏业务':'客户端与游戏'},
  PLTR:{'美国商业':'商业业务','美国政府':'政府业务'},
  LLY:{'Zepbound':'重点新品'}
};
function businessGrowthSection(s) {
  ...
  if(data.segments){
    ...
    const amountLabel=value=>Number.isFinite(Number(value))?`${data.currency||'$'}${Number(value).toFixed(Number(value)<1?2:1)}${data.amountUnit||'B'}`:String(value);
    const legend=data.segments.map(([name,value,,note],index)=>{const share=Number.isFinite(Number(value))&&total?`${(Number(value)/total*100).toFixed(1)}%`:'未披露';return `<div><i style="background:${colors[index%colors.length]}"></i><span><strong>${name}</strong><em>${note}</em></span><b>${share}</b><small>${amountLabel(value)}</small></div>`}).join('');
    const mix=numeric.length?`<div class="business-mix"><div class="business-pie" style="background:conic-gradient(${slices})" role="img" aria-label="${s.name}业务营收占比"></div><div class="business-pie-legend">${legend}</div></div>`:`<div class="business-pie-legend business-pie-legend-only">${legend}</div>`;
    const growth=data.segments.map(([name,value,growth])=>`<div><span>${name}</span><b class="${String(growth).includes('−')||String(growth).includes('-')?'negative':''}">${growth}</b><small>${Number.isFinite(Number(value))?`收入 ${amountLabel(value)}`:value}</small></div>`).join('');
    const visibleSubmetrics=(data.submetrics||[]).filter(([,growth])=>!['未单列','未披露','—','-'].includes(String(growth).trim()));
    const submetrics=visibleSubmetrics.length?`<div class="business-submetrics"><h4>重点子业务</h4>${visibleSubmetrics.map(([name,growth,note,parentPercent])=>{const parentName=SUBMETRIC_PARENT[s.ticker]?.[name]||data.segments.find(([segmentName])=>String(note).includes(segmentName))?.[0],parentShare=parentName&&Number.isFinite(Number(parentPercent))?` · 占${parentName} ${(Number(parentPercent)).toFixed(1)}%`:'';return `<div><span>${name}</span><b>${growth}</b><small>${note}${parentShare}</small></div>`}).join('')}</div>`:'';
    return `<section class="business-growth-card"><div class="business-growth-head"><div><p class="eyebrow">BUSINESS GROWTH</p><h3>业务营收占比与增长</h3></div><small>${data.period}</small></div>${mix}<h4 class="business-growth-subtitle">各一级业务同比增长率</h4><div class="business-growth-grid">${growth}</div>${submetrics}<p class="business-growth-source">来源：${data.source} · 财报发布于 ${data.issued}。</p></section>`;
  }
  const colors=['#2563eb','#f59e0b','#10b981','#8b5cf6','#ef4444','#06b6d4','#84cc16'],total=(data.mix||[]).reduce((sum,item)=>sum+Number(item[1]),0);let cursor=0;
  const slices=(data.mix||[]).map((item,index)=>{const start=cursor,end=cursor+(Number(item[1])/total*100);cursor=end;return `${colors[index%colors.length]} ${start.toFixed(2)}% ${end.toFixed(2)}%`}).join(',');
  const mix=data.mix?.length?`<div class="business-mix"><div class="business-pie" style="background:conic-gradient(${slices})" role="img" aria-label="${s.name}业务营收占比"></div><div class="business-pie-legend">${data.mix.map(([name,value],index)=>`<div><i style="background:${colors[index%colors.length]}"></i><span>${name}</span><b>${(Number(value)/total*100).toFixed(1)}%</b><small>$${Number(value).toFixed(value<1?2:1)}B</small></div>`).join('')}</div></div>`:'';
  return `<section class="business-growth-card"><div class="business-growth-head"><div><p class="eyebrow">BUSINESS GROWTH</p><h3>业务营收占比与增长</h3></div><small>${data.period}</small></div>${mix}<h4 class="business-growth-subtitle">各业务同比增长率</h4><div class="business-growth-grid">${data.items.map(([name,revenue,growth])=>`<div><span>${name}</span><b class="${String(growth).includes('−')||String(growth).includes('-')?'negative':''}">${growth}</b><small>${revenue}</small></div>`).join('')}</div><p class="business-growth-source">来源：${data.source} · 财报发布于 ${data.issued}。饼图仅采用互不重叠的营收分部；同比均与上年同期同口径比较。</p></section>`;
}
const companyTargetMargin = (ticker, fallback) => Number(COMPANY_MODEL_PROFILES[ticker]?.targetMargin ?? fallback);
const analystGrowthInfo = (m, fallback, ticker) => {
  const consensus = m.analystConsensus, path = Array.isArray(consensus?.growthPath) ? consensus.growthPath.slice(0,5).map(Number) : [];
  if (path.length === 5 && path.every(Number.isFinite)) return { growth:path, cagr:Number(consensus.revenueCagr), label:`前3年分析师共识${consensus.analystCount ? ` · ${consensus.analystCount}人` : ''}；第4–5年公司专属`, updatedAt:consensus.updatedAt };
  const far = COMPANY_FAR_GROWTH[ticker], growth = far ? [Number(fallback),Number(fallback),Number(fallback),...far] : fallback;
  return { growth, cagr:growthCagr(growth), label:far ? '前3年公司级回退；第4–5年公司专属' : '公司级回退假设', updatedAt:null };
};
const adjustedGrowth = (growth, adjustment) => Array.isArray(growth) ? growth.map(value => Math.max(-.30, Math.min(1, value + adjustment))) : Math.max(-.30, Math.min(1, Number(growth) + adjustment));
const growthCagr = growth => Array.isArray(growth) ? growth.reduce((product,value)=>product*(1+value),1) ** (1 / growth.length) - 1 : Number(growth);
...
const stockPriceNumber = value => { const number = Number.parseFloat(String(value || '').replace(/[$,]/g, '')); return Number.isFinite(number) ? number : null };
function financialResidualValue(s, m, overrides = {}) {
  const f = { ...(m.financialValuationModel || {}), ...overrides }, book = Number(f.commonEquityUsd), shares = Number(f.commonSharesOutstanding), income = Number(m.netIncomeTTM), cost = Number(overrides.costOfEquity ?? m.costOfEquity), terminalGrowth = Number(overrides.terminalGrowth ?? m.terminalGrowth), sustainableRoe = Number(overrides.sustainableRoe ?? m.financialSustainableRoe ?? f.sustainableRoe), bookGrowth = Number(overrides.bookValueGrowth ?? m.financialBookValueGrowth ?? f.bookValueGrowth);
  if (![book, shares, income, cost, terminalGrowth, sustainableRoe, bookGrowth].every(Number.isFinite) || book <= 0 || shares <= 0 || cost <= terminalGrowth || sustainableRoe <= cost) return null;
  const currentRoe = income / book, forecastYears = 10; let value = book, forecastBook = book;
  for (let year = 1; year <= forecastYears; year++) { const roe = currentRoe + (sustainableRoe - currentRoe) * year / forecastYears; value += forecastBook * (roe - cost) / (1 + cost) ** year; forecastBook *= 1 + bookGrowth }
  value += forecastBook * (sustainableRoe - cost) / (cost - terminalGrowth) / (1 + cost) ** forecastYears;
  return { equityValue: value, perShare: value / shares, currentRoe, priceToBook: marketCapNumber(s.cap) / book };
}
const financialSensitivity = (s, m) => {
  const f = m.financialValuationModel || {}, sustainableRoe = Number(m.financialSustainableRoe ?? f.sustainableRoe), bookGrowth = Number(m.financialBookValueGrowth ?? f.bookValueGrowth), scenarios = [
    { sustainableRoe: sustainableRoe - .04, bookValueGrowth: Math.max(0, bookGrowth - .02), costOfEquity: Number(m.costOfEquity) + .01, terminalGrowth: Math.max(0, Number(m.terminalGrowth) - .005) },
    {},
    { sustainableRoe: sustainableRoe + .04, bookValueGrowth: bookGrowth + .02, costOfEquity: Number(m.costOfEquity) - .01, terminalGrowth: Number(m.terminalGrowth) + .005 },
  ].map(overrides => financialResidualValue(s, m, overrides)?.perShare).filter(Number.isFinite);
  return scenarios.length ? `$${Math.min(...scenarios).toFixed(0)}–$${Math.max(...scenarios).toFixed(0)}` : '—';
};
const financialBusinessScenarios = (s, m) => [
  ['信贷型', financialResidualValue(s, m, { sustainableRoe: .28, bookValueGrowth: .05 })],
  ['闭环混合型', financialResidualValue(s, m, { sustainableRoe: .34, bookValueGrowth: .07 })],
  ['支付网络型', financialResidualValue(s, m, { sustainableRoe: .38, bookValueGrowth: .08 })],
];
function reverseImpliedGrowth(s, m) {
  if (m.modelApplicability === 'not-applicable-financial') return null;
  const marketCap = marketCapNumber(s.cap), revenue = Number(m.revenueTTM), currentMargin = Number(m.displayScenarioCurrentMargin ?? m.fcfMarginTTM), targetMargin = Number(m.displayScenarioTargetMargin ?? m.normalizedFcfMargin), cost = Number(m.costOfEquity), terminal = Number(m.terminalGrowth);
...
function reverseScenarioGrowth(s, m, normalizationYears = null, targetMargin = null, currentMarginOverride = null, costOverride = null) {
  if (m.modelApplicability === 'not-applicable-financial') return null;
  const marketCap = marketCapNumber(s.cap), revenue = Number(m.revenueTTM), currentMargin = Number(currentMarginOverride ?? m.fcfMarginTTM), historicalMargin = Number(targetMargin ?? m.fcfMargin3yMedian), cost = Number(costOverride ?? m.costOfEquity), terminal = Number(m.terminalGrowth), adjustment = Number(m.equityValueAdjustmentUsd) || 0;
...
function scenarioModelPrice(s, m, growth, targetMargin = null, normalizationYears = 5, currentMarginOverride = null, costOverride = null) {
  const profile = COMPANY_MODEL_PROFILES[s.ticker] || {}, marketCap = marketCapNumber(s.cap), price = stockPriceNumber(s.price), revenue = Number(m.revenueTTM), currentMargin = Number(currentMarginOverride ?? m.fcfMarginTTM), terminalMargin = Number(targetMargin ?? profile.targetMargin ?? m.normalizedFcfMargin), cost = Number(costOverride ?? m.valuationCostOfEquity ?? profile.valuationCost ?? m.costOfEquity), terminal = Number(m.terminalGrowth), adjustment = Number(m.equityValueAdjustmentUsd) || 0;
  let growthPath = Array.isArray(growth) ? growth.map(Number) : Array(5).fill(Number(growth));
  if (profile.tail && growthPath.length <= 5) growthPath = [...growthPath.slice(0,5), ...profile.tail];
  const forecastYears = growthPath.length, marginYears = Math.max(1, Number(profile.years || normalizationYears));
  if (![marketCap, price, revenue, currentMargin, terminalMargin, cost, terminal, ...growthPath].every(Number.isFinite) || forecastYears < 5 || marketCap <= 0 || price <= 0 || revenue <= 0 || terminalMargin < .03 || cost <= terminal) return null;
  const shares = marketCap / price, marginForYear = year => currentMargin + (terminalMargin - currentMargin) * Math.min(year / marginYears, 1);
  let operatingValue = 0, forecastRevenue = revenue; for (let year = 1; year <= forecastYears; year++) { forecastRevenue *= 1 + growthPath[year - 1]; operatingValue += forecastRevenue * marginForYear(year) / (1 + cost) ** year }
  operatingValue += forecastRevenue * terminalMargin * (1 + terminal) / (cost - terminal) / (1 + cost) ** forecastYears;
  const equityValue = operatingValue + adjustment;
  return equityValue > 0 ? equityValue / shares : null;
}
const priceLabel = value => Number.isFinite(value) ? `$${value.toFixed(0)}` : '—';
const priceGapLabel = (value, s) => { const price = stockPriceNumber(s.price); return Number.isFinite(value) && price ? `${value >= price ? '+' : ''}${((value / price - 1) * 100).toFixed(0)}%` : '—' };
...
const referenceFcfRate = (s, m) => {
  const special = SPECIAL_VALUATION_SCENARIOS[s.ticker], cycleReady = CYCLE_SCENARIO_TICKERS.has(s.ticker) && Number(m.fcfMarginCycleYears) >= 5;
  const rate = Number(m.displayFcfRate ?? (special ? special.scenarios[1][1] : cycleReady ? m.fcfMarginCycleMedian : m.normalizedFcfMargin));
  return Number.isFinite(rate) && rate >= .03 ? rate : null;
};
function currentPriceRequiredGrowth(s, m) {
  if (m.modelApplicability === 'not-applicable-financial') return null;
  const marketCap = marketCapNumber(s.cap), revenue = Number(m.revenueTTM), fcfRate = referenceFcfRate(s,m), cost = Number(m.costOfEquity), terminal = Number(m.terminalGrowth), adjustment = Number(m.equityValueAdjustmentUsd) || 0, years = Math.max(1,Math.min(20,Math.round(Number(m.forecastYears)||5))), target = marketCap - adjustment;
  if (![marketCap,revenue,fcfRate,cost,terminal,target].every(Number.isFinite) || target <= 0 || cost <= terminal) return null;
  const value = growth => { let total=0, forecast=revenue; for(let year=1;year<=years;year++){forecast*=1+growth;total+=forecast*fcfRate/(1+cost)**year} total+=forecast*fcfRate*(1+terminal)/(cost-terminal)/(1+cost)**years; return total };
  let low=-.30,high=1.50; for(let i=0;i<64;i++){const mid=(low+high)/2;if(value(mid)<target)low=mid;else high=mid}
  return value(high)>=target ? high : null;
}
function simpleGrowthCards(s,m) {
  const required=currentPriceRequiredGrowth(s,m),fcfRate=referenceFcfRate(s,m),analystExpectation=ANALYST_REVENUE_EXPECTATIONS[s.ticker],guidance=COMPANY_REVENUE_GUIDANCE[s.ticker],forwardReference=analystExpectation||guidance;
  const forecastYears=Math.max(1,Math.min(20,Math.round(Number(m.forecastYears)||5))),requiredValue=Number.isFinite(required)?`${(required*100).toFixed(0)}%`:'—', requiredNote=m.modelApplicability==='not-applicable-financial'?'金融公司不适用收入FCF反推':fcfRate===null?'缺少可持续的公司级FCF率':`未来${forecastYears}年收入CAGR；关键条件可在下方编辑`;
  const forwardLabel=analystExpectation?'分析师收入预期':'公司财报指引',guidanceValue=forwardReference?.value||'',guidanceNote=analystExpectation?`${analystExpectation.period} · ${analystExpectation.source}，更新于 ${analystExpectation.updated}`:guidance?`${guidance.period} · 财报发布于 ${guidance.issued}`:'';
  const hasForwardValue=Boolean(forwardReference)&&!['','未提供','未量化','—','-'].includes(String(guidanceValue).trim());
  const forwardCard=hasForwardValue?`<div><span>${forwardLabel}</span><b>${guidanceValue}</b><small>${guidanceNote}</small></div>`:'';
  const quarter=reportedQuarterLabel(s),actual=s.growth||'—';
  const ttmFcf=Number(m.fcfMarginTTM),median3y=Number(m.fcfMargin3yMedian),weight=Number(m.ttmWeight),cycleReady=CYCLE_SCENARIO_TICKERS.has(s.ticker)&&Number(m.fcfMarginCycleYears)>=5,fcfDefaultSource=SPECIAL_VALUATION_SCENARIOS[s.ticker]?'默认：公司级正常化假设':cycleReady?'默认：5–7年周期中位数':'默认：TTM与近3年中位数';
  const fcfMethod=SPECIAL_VALUATION_SCENARIOS[s.ticker]?`实际值：TTM ${(ttmFcf*100).toFixed(1)}%，近3年中位数 ${(median3y*100).toFixed(1)}%。默认算法：根据该公司现金流正常化情景单独设为 ${(fcfRate*100).toFixed(1)}%，属于模型假设，不是财报指引。`:cycleReady?`实际值：TTM ${(ttmFcf*100).toFixed(1)}%，近3年中位数 ${(median3y*100).toFixed(1)}%，${Number(m.fcfMarginCycleYears)}年周期中位数 ${(Number(m.fcfMarginCycleMedian)*100).toFixed(1)}%。默认算法：采用完整周期中位数，即 ${(fcfRate*100).toFixed(1)}%。`:`实际值：TTM ${(ttmFcf*100).toFixed(1)}%，近3年中位数 ${(median3y*100).toFixed(1)}%。默认算法：TTM × ${(weight*100).toFixed(0)}% + 三年中位数 × ${((1-weight)*100).toFixed(0)}% = ${(fcfRate*100).toFixed(1)}%。`;
  const growthFormula=m.modelApplicability==='not-applicable-financial'?'金融公司不使用这套收入—FCF反推公式。':`计算公式：第 t 年收入 = TTM收入 × (1 + g)^t；第 t 年FCF = 第 t 年收入 × 假设FCF率；终值 = 第 ${forecastYears} 年FCF × (1 + 永续增长率) ÷ (权益成本 − 永续增长率)；企业经营价值 = 各年FCF现值 + 终值现值；再加净现金等权益调整，与当前市值相等时反解 g。`;
  const controls=m.modelApplicability==='not-applicable-financial'?'':`<div class="simple-growth-controls"><label><span>假设FCF率<small class="control-source">${fcfDefaultSource}</small></span><span><input type="number" inputmode="decimal" data-model-input="displayFcfRate" value="${(fcfRate*100).toFixed(1)}" min="3" max="80" step="0.1"><small>%</small></span></label><label><span>Beta<small class="control-source">CAPM风险参数</small></span><span><input type="number" inputmode="decimal" data-model-input="beta" value="${Number(m.beta).toFixed(2)}" min="0" max="5" step="0.01"></span></label><label><span>权益成本<small class="control-source">修改Beta时自动重算</small></span><span><input type="number" inputmode="decimal" data-model-input="costOfEquity" value="${(Number(m.costOfEquity)*100).toFixed(1)}" min="1" max="30" step="0.1"><small>%</small></span></label><label><span>永续增长</span><span><input type="number" inputmode="decimal" data-model-input="terminalGrowth" value="${(Number(m.terminalGrowth)*100).toFixed(1)}" min="0" max="6" step="0.1"><small>%</small></span></label><label><span>预测年限</span><span><input type="number" inputmode="numeric" data-model-input="forecastYears" value="${forecastYears}" min="1" max="20" step="1"><small>年</small></span></label></div><p class="fcf-method-note">CAPM：权益成本 = 无风险利率 ${(Number(m.riskFreeRate)*100).toFixed(2)}% + Beta <span id="capm-beta-value">${Number(m.beta).toFixed(2)}</span> × 股权风险溢价 ${(Number(m.equityRiskPremium)*100).toFixed(2)}%。权益成本仍可直接覆盖。</p>`;
  return `<div class="model-head simple-growth-head"><div><p class="eyebrow">PRICE &amp; GROWTH CHECK</p><h3>价格与增长对照</h3></div><div class="growth-compare growth-scenarios simple-growth-cards"><div class="baseline-primary"><span>当前价格要求的增长</span><b id="simple-required-growth">${requiredValue}</b><small id="simple-required-note">${requiredNote}</small></div>${forwardCard}<div><span>当前季度增长</span><b>${actual}</b><small>${quarter}已披露财报 · 对比去年同期收入</small></div></div></div>${controls}${m.modelApplicability==='not-applicable-financial'?'':`<p class="fcf-method-note">${fcfMethod}</p>`}<p class="model-formula-note">${growthFormula}</p>`;
}
function modelInputs(s) {
  ...
  return simpleGrowthCards(s,m);
  if (m.modelApplicability === 'not-applicable-financial') {
    const f = m.financialValuationModel || {}, result = financialResidualValue(s, m), marketPrice = stockPriceNumber(s.price), upside = result && marketPrice ? (result.perShare / marketPrice - 1) * 100 : null, businessScenarios = financialBusinessScenarios(s, m);
    if (!result) return `<p class="eyebrow">RESIDUAL-INCOME MODEL</p><h3>金融公司剩余收益估值</h3><p class="model-result-unavailable">缺少可核验的普通股权益或股本数据，暂不计算。普通工业公司的自由现金流模型不会用于美国运通。</p>`;
    return `<div class="model-head"><div><p class="eyebrow">RESIDUAL-INCOME MODEL</p><h3>闭环支付与信贷混合估值</h3></div><div class="growth-compare growth-scenarios financial-business-scenarios">${businessScenarios.map(([label, scenario], index) => `<div class="${index === 1 ? 'baseline-primary' : ''}"><span>${label}</span><b data-financial-scenario="${index}">${scenario ? `$${scenario.perShare.toFixed(0)}` : '—'}</b><small>可持续 ROE ${[28, 34, 38][index]}%</small></div>`).join('')}</div></div><div class="model-grid"><span><span class="model-metric-name">当前 P/B</span><b>${result.priceToBook.toFixed(1)}x</b></span><span><span class="model-metric-name">TTM ROE</span><b>${(result.currentRoe * 100).toFixed(1)}%</b></span><span><span class="model-metric-name">基准合理价值</span><b id="financial-fair-value">$${result.perShare.toFixed(0)}</b></span><span><span class="model-metric-name">基准相对现价</span><b id="financial-upside">${upside === null ? '—' : `${upside >= 0 ? '+' : ''}${upside.toFixed(0)}%`}</b></span><span><span class="model-metric-name">敏感度区间</span><b id="financial-sensitivity">${financialSensitivity(s, m)}</b></span><span><span class="model-metric-name">普通股权益</span><b>$${(Number(f.commonEquityUsd) / 1e9).toFixed(1)}B</b></span>${editableAssumption('可持续 ROE', 'financialSustainableRoe', m.financialSustainableRoe ?? f.sustainableRoe, { percent: true, min: 5, max: 60 })}${editableAssumption('账面价值增长', 'financialBookValueGrowth', m.financialBookValueGrowth ?? f.bookValueGrowth, { percent: true, min: -10, max: 30 })}${editableAssumption('股权成本', 'costOfEquity', m.costOfEquity, { percent: true, min: 1, max: 30 })}${editableAssumption('永续增长', 'terminalGrowth', m.terminalGrowth, { percent: true, min: 0, max: 6 })}</div><p class="model-assumption-hint">美国运通同时拥有闭环支付网络、商户折扣收入、会员费和消费信贷。三种情景分别反映更接近发卡信贷、当前混合结构以及更接近支付网络的长期盈利能力。</p><p class="model-formula-note">价值 = 当前普通股权益 + 未来 10 年超额收益现值 + 终值；超额收益 = 期初普通股权益 ×（ROE − 股权成本）。</p><p class="model-note">权益与股本截止：${f.periodEnd || m.fiscalPeriodEnd}。${f.source || 'SEC 公司财报'}；净利润采用当前 TTM 数据。结果是模型情景，不是目标价。</p>`;
  }
  ...
  const special = SPECIAL_VALUATION_SCENARIOS[s.ticker];
  if (special) {
    const valuationCost = Number(m.valuationCostOfEquity ?? special.valuationCostOfEquity) || null, consensus = analystGrowthInfo(m, special.scenarios[1][2], s.ticker), hasConsensus = Array.isArray(consensus.growth), results = special.scenarios.map(([, margin, fallbackGrowth],index) => { const growth=hasConsensus?adjustedGrowth(consensus.growth,[-.05,0,.05][index]):fallbackGrowth; return { required:scenarioLabel([reverseScenarioGrowth(s,m,5,margin,special.currentMargin,valuationCost)]),price:scenarioModelPrice(s,m,growth,margin,5,special.currentMargin,valuationCost),growth } });
    heading = special.title;
    comparison = `<div class="growth-compare growth-scenarios special-company-scenarios">${special.scenarios.map(([label], index) => `<div class="${index === 1 ? 'baseline-primary' : ''}"><span>${label}</span><b data-special-scenario="${index}">${priceLabel(results[index].price)}</b><small>${consensus.label} ${(growthCagr(results[index].growth)*100).toFixed(1)}% · 隐含要求 ${results[index].required}</small><em>${priceGapLabel(results[index].price, s)} vs ${s.price}</em></div>`).join('')}</div>`;
    guidance = special.note;
  } else if (CAPEX_SCENARIO_TICKERS.has(s.ticker)) {
    const growthInfo=analystGrowthInfo(m,REFERENCE_GROWTH[s.ticker]??.15,s.ticker),growth=growthInfo.growth,currentRequired=scenarioLabel([reverseScenarioGrowth(s,m)]),normalizedRequired=scenarioLabel([reverseScenarioGrowth(s,m,3),reverseScenarioGrowth(s,m,5)]),current=scenarioModelPrice(s,m,growth,m.fcfMarginTTM),normalized=scenarioModelPrice(s,m,growth,companyTargetMargin(s.ticker,m.fcfMargin3yMedian),4);
    heading = '两种资本开支情景';
    comparison = `<div class="growth-compare growth-scenarios" aria-label="资本开支双情景估值对比"><div><span>当前资本开支持续 ${modelHelpButton('capex-current', '当前资本开支持续')}</span><b id="scenario-current-growth">${priceLabel(current)}</b><small>${growthInfo.label} ${(growthCagr(growth)*100).toFixed(1)}% · 隐含要求 ${currentRequired}</small><em>${priceGapLabel(current,s)} vs ${s.price}</em></div><div class="baseline-primary"><span>资本开支正常化 ${modelHelpButton('capex-normalized', '资本开支正常化')}</span><b id="scenario-normalized-growth">${priceLabel(normalized)}</b><small>${growthInfo.label} ${(growthCagr(growth)*100).toFixed(1)}% · 隐含要求 ${normalizedRequired}</small><em>${priceGapLabel(normalized,s)} vs ${s.price}</em></div><i aria-hidden="true">对比</i>${actual}</div>`;
    ...
    const growthInfo=analystGrowthInfo(m,REFERENCE_GROWTH[s.ticker]??.12,s.ticker),growth=growthInfo.growth,currentRequired=scenarioLabel([reverseScenarioGrowth(s,m)]),normalizedRequired=cycleReady?scenarioLabel([reverseScenarioGrowth(s,m,5,cycleMargin),reverseScenarioGrowth(s,m,7,cycleMargin)]):'—',current=scenarioModelPrice(s,m,growth,m.fcfMarginTTM),normalized=cycleReady?scenarioModelPrice(s,m,growth,companyTargetMargin(s.ticker,cycleMargin),6):null;
    heading = '完整投资周期情景'; medianLabel = '5–7年周期中位数'; medianValue = cycleMargin;
    comparison = `<div class="growth-compare growth-scenarios" aria-label="投资周期双情景估值对比"><div><span>当前投资周期 ${modelHelpButton('cycle-current', '当前投资周期')}</span><b id="scenario-current-growth">${priceLabel(current)}</b><small>${growthInfo.label} ${(growthCagr(growth)*100).toFixed(1)}% · 隐含要求 ${currentRequired}</small><em>${priceGapLabel(current,s)} vs ${s.price}</em></div><div class="baseline-primary"><span>完整周期正常化 ${modelHelpButton('cycle-normalized', '完整周期正常化')}</span><b id="scenario-cycle-growth">${priceLabel(normalized)}</b><small>${cycleReady ? `${growthInfo.label} ${(growthCagr(growth)*100).toFixed(1)}% · 隐含要求 ${normalizedRequired}` : cycleFieldsPresent ? `当前仅有${Number(m.fcfMarginCycleYears) || 0}个完整年度` : '周期数据尚未回填'}</small><em>${priceGapLabel(normalized,s)} vs ${s.price}</em></div><i aria-hidden="true">对比</i>${actual}</div>`;
    ...
    const growthInfo=analystGrowthInfo(m,REFERENCE_GROWTH[s.ticker]??.12,s.ticker),growth=growthInfo.growth,required=reverseImpliedGrowth(s,m)||'—',profileMargin=companyTargetMargin(s.ticker,m.normalizedFcfMargin),baselinePrice=scenarioModelPrice(s,m,growth,profileMargin),lowPrice=scenarioModelPrice(s,m,adjustedGrowth(growth,-.03),profileMargin*.8),highPrice=scenarioModelPrice(s,m,adjustedGrowth(growth,.03),profileMargin*1.2),sensitivity=[lowPrice,highPrice].filter(Number.isFinite);
    heading = '公司级基准估值';
    comparison = `<div class="growth-compare growth-scenarios baseline-comparison" aria-label="公司级模型估值"><div class="baseline-primary"><span>基准模型价值 ${modelHelpButton('required-growth', '模型每股价值')}</span><b id="model-required-growth">${priceLabel(baselinePrice)}</b><small>${growthInfo.label} ${(growthCagr(growth)*100).toFixed(1)}% · 隐含要求 ${required}</small><em>${priceGapLabel(baselinePrice,s)} vs ${s.price}</em></div><div><span>模型敏感度 ${modelHelpButton('growth-sensitivity', '模型敏感度')}</span><b id="model-growth-sensitivity">${sensitivity.length ? `${priceLabel(Math.min(...sensitivity))}–${priceLabel(Math.max(...sensitivity))}` : '—'}</b><small>共识增长与现金流率联动范围</small></div><i aria-hidden="true">对比</i>${actual}</div>`;
  ...
  const normalizedMargin = Number(m.normalizedFcfMargin), normalizedReliable = Number.isFinite(normalizedMargin) && normalizedMargin >= .03;
  const normalizedDisplay = normalizedReliable ? pct(normalizedMargin) : '—';
  const reliabilityNote = normalizedReliable ? '' : '<p class="model-result-unavailable">当前归一化自由现金流率低于 3% 或为负，不具备可持续终值含义，因此不作为公司级估值基准。</p>';
  const consensusMeta = m.analystConsensus, farGrowth = COMPANY_FAR_GROWTH[s.ticker], profile = COMPANY_MODEL_PROFILES[s.ticker], farGrowthNote = farGrowth ? ` 第4年/第5年收入增速采用公司专属估值假设：${pct(farGrowth[0])}/${pct(farGrowth[1])}，不是分析师共识或公司指引。` : '', profileNote = profile ? ` 显性预测期：${profile.years}年；${profile.note}。` : ' 显性预测期：5年。', consensusNote = consensusMeta?.updatedAt ? ` 分析师收入共识：Alpha Vantage，${consensusMeta.analystCount ? `${consensusMeta.analystCount}位分析师，` : ''}更新于 ${String(consensusMeta.updatedAt).slice(0,10)}。${farGrowthNote}${profileNote}` : ` 当前无可用分析师收入共识，前3年使用公司级回退假设。${farGrowthNote}${profileNote}`;
  const profileCost = special?.valuationCostOfEquity ?? profile?.valuationCost, costInput = profileCost ? editableAssumption('估值权益成本', 'valuationCostOfEquity', m.valuationCostOfEquity ?? profileCost, { percent: true, min: 0.1, max: 50 }) : editableAssumption('权益成本', 'costOfEquity', m.costOfEquity, { percent: true, min: 0.1, max: 50 });
  return `<div class="model-head"><div><p class="eyebrow">MODEL VALUE SCENARIOS</p><h3>${heading}</h3></div>${comparison}</div><div class="model-grid"><span><span class="model-metric-name">TTM 自由现金流率 ${modelHelpButton('fcf-ttm', 'TTM 自由现金流率')}</span><b>${pct(m.fcfMarginTTM)}</b></span><span><span class="model-metric-name">${medianLabel} ${modelHelpButton('fcf-median', medianLabel)}</span><b>${medianDisplay}</b></span><span><span class="model-metric-name">公司级基准 FCF 率 ${modelHelpButton('fcf-normalized', '公司级基准 FCF 率')}</span><b>${normalizedDisplay}</b></span>${costInput}${editableAssumption('Beta（参考）', 'beta', m.beta, { step: '0.01', min: 0, max: 5 })}${editableAssumption('永续增长', 'terminalGrowth', m.terminalGrowth, { percent: true, min: -5, max: 10 })}</div>${reliabilityNote}<p class="model-assumption-hint">${guidance} 调整估值权益成本或永续增长后会立即重算。</p><p class="model-formula-note">主结果是参考增长与现金流情景对应的模型每股价值，并与当前价格比较；隐含增长要求保留为辅助信息。所有结果均为模型情景，不是目标价或公司指引。</p><p class="model-note">截止财报期：${m.fiscalPeriodEnd}。${m.rationale}${consensusNote} 财报来源：${m.source}。</p>`;
}
function updateModelGrowthPreview() {
  ...
  const m = modelForStock(s), values = {}; container.querySelectorAll('[data-model-input]').forEach(input => { const n = Number(input.value), key=input.dataset.modelInput; if (Number.isFinite(n)) values[key] = key === 'beta' || key === 'forecastYears' ? n : n / 100 });
  Object.assign(m, values);
  const simpleRequired=container.querySelector('#simple-required-growth'),simpleNote=container.querySelector('#simple-required-note');
  if(simpleRequired){const required=currentPriceRequiredGrowth(s,m),years=Math.max(1,Math.min(20,Math.round(Number(m.forecastYears)||5)));simpleRequired.textContent=Number.isFinite(required)?`${(required*100).toFixed(0)}%`:'—';if(simpleNote)simpleNote.textContent=m.modelApplicability==='not-applicable-financial'?'金融公司不适用收入FCF反推':referenceFcfRate(s,m)===null?'缺少可持续的公司级FCF率':`未来${years}年收入CAGR；关键条件可在下方编辑`;return}
  ...
  const referenceGrowth = analystGrowthInfo(m,REFERENCE_GROWTH[s.ticker]??.12,s.ticker).growth;
  if (current) current.textContent = priceLabel(scenarioModelPrice(s,m,referenceGrowth,m.fcfMarginTTM));
  if (normalized) normalized.textContent = priceLabel(scenarioModelPrice(s,m,referenceGrowth,companyTargetMargin(s.ticker,m.fcfMargin3yMedian),4));
  if (cycle && Number(m.fcfMarginCycleYears) >= 5) cycle.textContent = priceLabel(scenarioModelPrice(s,m,referenceGrowth,companyTargetMargin(s.ticker,m.fcfMarginCycleMedian),6));
  const previewMargin=companyTargetMargin(s.ticker,m.normalizedFcfMargin);
  if (baseline) baseline.textContent = priceLabel(scenarioModelPrice(s,m,referenceGrowth,previewMargin));
  if (sensitivity) { const prices=[scenarioModelPrice(s,m,adjustedGrowth(referenceGrowth,-.03),previewMargin*.8),scenarioModelPrice(s,m,adjustedGrowth(referenceGrowth,.03),previewMargin*1.2)].filter(Number.isFinite); sensitivity.textContent=prices.length?`${priceLabel(Math.min(...prices))}–${priceLabel(Math.max(...prices))}`:'—' }
  const financialFairValue = container.querySelector('#financial-fair-value'), financialRange = container.querySelector('#financial-sensitivity'), financialUpside = container.querySelector('#financial-upside');
  if (financialFairValue) { const result = financialResidualValue(s, m), marketPrice = stockPriceNumber(s.price); financialFairValue.textContent = result ? `$${result.perShare.toFixed(0)}` : '—'; if (financialRange) financialRange.textContent = financialSensitivity(s, m); if (financialUpside) financialUpside.textContent = result && marketPrice ? `${result.perShare >= marketPrice ? '+' : ''}${((result.perShare / marketPrice - 1) * 100).toFixed(0)}%` : '—' }
  container.querySelectorAll('[data-financial-scenario]').forEach((node, index) => { const scenario = financialBusinessScenarios(s, m)[index]?.[1]; node.textContent = scenario ? `$${scenario.perShare.toFixed(0)}` : '—' });
  const special = SPECIAL_VALUATION_SCENARIOS[s.ticker]; if (special) { const consensus=analystGrowthInfo(m,special.scenarios[1][2],s.ticker),hasConsensus=Array.isArray(consensus.growth); container.querySelectorAll('[data-special-scenario]').forEach((node,index)=>{const [,margin,fallbackGrowth]=special.scenarios[index]||[],growth=hasConsensus?adjustedGrowth(consensus.growth,[-.05,0,.05][index]):fallbackGrowth,valuationCost=Number(m.valuationCostOfEquity??special.valuationCostOfEquity)||null;node.textContent=priceLabel(scenarioModelPrice(s,m,growth,margin,5,special.currentMargin,valuationCost))}) }
}
document.querySelector('#model-inputs').addEventListener('input', event => {
  ...
  if (input.dataset.modelInput === 'forecastYears') {
    override.forecastYears = Math.max(1, Math.min(20, Math.round(value)));
  } else if (input.dataset.modelInput === 'beta') {
    override.beta = value;
    const betaValue = document.querySelector('#capm-beta-value'); if (betaValue) betaValue.textContent = value.toFixed(2);
...
document.querySelector('#model-inputs').addEventListener('click', event => {
  const valueInput = event.target instanceof HTMLInputElement && event.target.matches('[data-model-input]') ? event.target : null;
  if (valueInput) { requestAnimationFrame(() => valueInput.select()); return }
...
document.querySelector('#model-inputs').addEventListener('focusin', event => {
  const input = event.target; if (!(input instanceof HTMLInputElement) || !input.matches('[data-model-input]')) return;
...
Object.assign(metricHelp, {
  ...
  'implied-growth': ['隐含增长率', `<p class="plain-lead">详情页按业务结构选择模型，不给所有公司强套同一种现金流率。</p><h3>云平台重投入</h3><p>Microsoft、Alphabet、Meta使用当前资本开支持续与正常化情景；Amazon和Oracle改用高投入、混合基准和成熟期现金流情景。</p><h3>周期与分部业务</h3><p>TSMC、Micron、SK hynix使用完整周期；NVIDIA、AMD增加半导体压力测试；Tesla区分汽车、能源和软件期权。</p><h3>特殊公司</h3><p>Palantir使用扣除股权激励稀释后的所有者收益；礼来采用药品组合与管线情景；美国运通使用剩余收益模型。</p><div class="beginner-tip">结果代表当前价格要求的增长，不是公司指引或目标价。</div>`],
...
const REPORTED_FISCAL_QUARTERS={
  ...
  AVGO:{'2026-05-03':'FY2026 Q2','2026-04-30':'FY2026 Q2'},
  ORCL:{'2026-05-31':'FY2026 Q4'}
};
const reportedQuarterLabel = stock => { const raw = stock?.ttmPeriodEnd || stock?.valuationModel?.fiscalPeriodEnd; if (!raw) return '最近已披露季度'; const fiscalLabel=REPORTED_FISCAL_QUARTERS[stock?.ticker]?.[raw]; if(fiscalLabel) return fiscalLabel; const date = new Date(`${raw}T00:00:00Z`); if (Number.isNaN(date.getTime())) return '最近已披露季度'; return `${date.getUTCFullYear()} Q${Math.floor(date.getUTCMonth() / 3) + 1}` };
const comparableQuarterLabel = stock => { const current=reportedQuarterLabel(stock); if(current==='最近已披露季度') return current; const previous=current.replace(/(FY)?(\d{4})/,(_,fy,year)=>`${fy||''}${Number(year)-1}`); return `${current} vs ${previous}` };
const refreshGrowthPanels = () => {
  ...
  document.querySelector('#detail-revenue-period').textContent = comparableQuarterLabel(stock);
