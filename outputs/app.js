...
function drawChart(s){
  ...
  const lines=metrics.map(m=>{const availableCount=points.filter(point=>numericValue(point[m[0]])).length;return availableCount>1?`<path class="line" stroke="${m[2]}" d="${historicalLinePath(points,m[0],min,max)}"/>`:''}).join('');
  const peGaps=[];let gapStart=null;points.forEach((point,index)=>{if(!numericValue(point.pe)&&gapStart===null)gapStart=index;if((numericValue(point.pe)||index===points.length-1)&&gapStart!==null){const end=numericValue(point.pe)?index-1:index;if(end-gapStart>=4)peGaps.push([gapStart,end]);gapStart=null}});
  const gapNotes=peGaps.map(([start,end])=>{const x=76+904*((start+end)/2)/Math.max(1,points.length-1);return `<g class="pe-loss-gap"><rect x="${(x-86).toFixed(1)}" y="69" width="172" height="28" rx="14" fill="#fff4e8" stroke="#f2c896" stroke-width="1"/><text x="${x.toFixed(1)}" y="87" text-anchor="middle" fill="#b46117" style="font:600 10px DM Mono,monospace">亏损期，PE（TTM）不适用</text></g>`}).join('');
  svg.innerHTML=`${grid}<line class="grid" x1="76" y1="314" x2="980" y2="314"/>${lines}${gapNotes}<rect class="chart-hover-overlay" x="76" y="62" width="904" height="252" fill="transparent"/><line class="hover-guide hover-guide-x" y1="62" y2="314" style="display:none"/><line class="hover-guide hover-guide-y" x1="76" x2="980" style="display:none"/><g class="hover-points"></g>`;
}
...
Promise.all([fetch('data/stocks.json',{cache:'no-store'}),fetch('data/history.json',{cache:'no-store'}),fetch('data/nasdaq.json',{cache:'no-store'}),fetch('data/concentration.json',{cache:'no-store'})]).then(async([market,history,nasdaq,concentration])=>{if(market.ok){const data=await market.json();if(Array.isArray(data.stocks)&&data.stocks.length){const fetched=data.stocks.filter(stock=>!['SPCX','SKHY'].includes(stock.ticker));const missing=fallbackStocks.filter(stock=>!fetched.some(item=>item.ticker===stock.ticker));stocks=[...fetched,...missing];lastUpdated=formatJapanTime(data.updatedAt)||lastUpdated;renderStocks();const stamp=document.querySelector('.updated b');if(stamp)stamp.textContent=lastUpdated}}if(history.ok){const data=await history.json();historyByTicker=data.stocks||{}}if(nasdaq.ok){nasdaqData=await nasdaq.json();renderNasdaq()}if(concentration.ok){concentrationData=await concentration.json();renderMag7Share()}syncDetailRoute();if(!document.querySelector('#detail').classList.contains('hidden'))drawSelectedChart(stocks[activeIndex])}).catch(()=>{});renderStocks();renderNasdaq();
...
const renderMobileStocks=()=>{
  ...
  target.innerHTML=ordered.map(stock=>{
    ...
    return `<article class="mobile-stock-card" data-ticker="${stock.ticker}">
      ...
      <div class="mobile-more-grid"><span><small>PEG <button class="mobile-metric-help" data-metric="peg" aria-label="PEG说明">?</button></small><b>${multiple(stock.peg)}</b></span><span><small>市销率 <button class="mobile-metric-help" data-metric="ps" aria-label="市销率说明">?</button></small><b>${multiple(stock.ps)}</b></span><span><small>市现率 <button class="mobile-metric-help" data-metric="pcf" aria-label="市现率说明">?</button></small><b>${multiple(stock.pcf)}</b></span><span><small>EV/EBITDA <button class="mobile-metric-help" data-metric="ev-ebitda" aria-label="EV EBITDA说明">?</button></small><b>${multiple(stock.evEbitda)}</b></span><span><small>隐含增长率 <button class="mobile-metric-help" data-metric="implied-growth" aria-label="隐含增长率说明">?</button></small><b>${stock.implied||'—'}</b></span><span><small>收入同比 <button class="mobile-metric-help" data-metric="revenue-growth" aria-label="收入同比说明">?</button></small><b>${stock.growth||'—'}</b></span><span><small>EPS 增长 <button class="mobile-metric-help" data-metric="eps-growth" aria-label="EPS增长说明">?</button></small><b class="${rankClass(stock,'epsGrowthCurrent')}">${stock.epsGrowthCurrent||'—'}</b></span></div>
    </article>`;
  }).join('');
