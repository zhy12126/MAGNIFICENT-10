...
let COMPANY_REVENUE_GUIDANCE = {};
let ANALYST_REVENUE_EXPECTATIONS = {};
let BUSINESS_GROWTH = {};
...
Promise.all([fetch('data/stocks.json', { cache: 'no-store' }), fetch('data/history.json', { cache: 'no-store' }), fetch('data/nasdaq.json', { cache: 'no-store' }), fetch('data/company-reporting.json', { cache: 'no-store' })]).then(async ([market, history, nasdaq, reporting]) => { if(reporting.ok){const data=await reporting.json();COMPANY_REVENUE_GUIDANCE=data.guidance||{};ANALYST_REVENUE_EXPECTATIONS=data.analystDisplayOverrides||{};BUSINESS_GROWTH_AUDITED=data.business||{};SUBMETRIC_PARENT=data.submetricParents||{}} if (market.ok) { const data = await market.json(); if (Array.isArray(data.stocks) && data.stocks.length) { const fetched = data.stocks; const missing = fallbackStocks.filter(stock => !fetched.some(item => item.ticker === stock.ticker)); stocks = [...fetched, ...missing]; lastUpdated = formatJapanTime(data.updatedAt) || lastUpdated; renderStocks(); const stamp = document.querySelector('.updated b'); if (stamp) stamp.textContent = lastUpdated } } if (history.ok) { const data = await history.json(); historyByTicker = data.stocks || {}; renderStocks() } if (nasdaq.ok) { nasdaqData = await nasdaq.json(); renderNasdaq() } marketDataReady = true; syncDetailRoute(); if (!document.querySelector('#detail').classList.contains('hidden')) drawSelectedChart(stocks[activeIndex]) }).catch(error => { console.warn('Unable to load primary market data', error); marketDataReady = true; syncDetailRoute() }); loadConcentration(); renderStocks();
