...
let stocks=fallbackStocks,historyByTicker={},nasdaqData=null,activeIndex=0,activePeriod='3年',activeValuationPeriod='3年',activePricePeriod='1年',lastUpdated='等待日更数据',activeNasdaqMetrics=new Set(['pe','forwardPe','pb']),sortKey='cap',sortDirection=-1,chartMode='valuation';
const rankingDirection={pe:1,fpe:1,peg:1,ps:1,evEbitda:1,implied:1,growth:-1};
...
function drawChart(s){
  ...
  const metrics=[['pe','PE（TTM）','var(--orange)','orange'],['pcf','Price / CF（TTM）','var(--teal)','teal'],['ps','Price / Sales（TTM）','var(--violet)','violet']];
  ...
  document.querySelector('#stats').innerHTML=current.map(v=>`<div class="stat"><div class="stat-label"><i class="${v[2]}"></i>${v[0]}</div><div class="stat-value">${v[1]}<span class="stat-sub">${v[1]==='—'?'':'x'}</span></div><div class="stat-sub">${statDate}</div></div>`).join('');
  ...
  document.querySelector('.chart-hover-hint').textContent=`按历史日收盘价和当时已公开的滚动四季财报重建；当前显示 ${range}。${availability}${limitNotice}`;
  ...
  const values=points.flatMap(p=>metrics.map(m=>p[m[0]]).filter(numericValue).map(Number));const min=Math.max(0,Math.floor(Math.min(...values)/5)*5),max=Math.ceil(Math.max(...values)/5)*5||5;let grid='';
  ...
  const lines=metrics.map(m=>{const series=points.filter(p=>numericValue(p[m[0]]));return series.length>1?`<path class="line" stroke="${m[2]}" d="${linePath(series,m[0],min,max)}"/>`:''}).join('');
  svg.innerHTML=`${grid}<line class="grid" x1="76" y1="314" x2="980" y2="314"/>${lines}<rect class="chart-hover-overlay" x="76" y="62" width="904" height="252" fill="transparent"/><line class="hover-guide hover-guide-x" y1="62" y2="314" style="display:none"/><line class="hover-guide hover-guide-y" x1="76" x2="980" style="display:none"/><g class="hover-points"></g>`;
}
...
function openDetail(ticker){const i=stocks.findIndex(stock=>stock.ticker===ticker);if(i<0)return;activeIndex=i;const s=stocks[i];document.querySelector('#overview').classList.add('hidden');document.querySelector('#detail').classList.remove('hidden');document.querySelector('#detail-logo').outerHTML=logo(s,true).replace('<div class="logo lg"','<div id="detail-logo" class="logo lg"');document.querySelector('#detail-ticker').textContent=s.ticker+' · NASDAQ';document.querySelector('#detail-name').textContent=s.name;document.querySelector('#detail-price').textContent=`${s.price}  ·  ${s.change||'—'}  ·  市值 $${s.cap}`;document.querySelector('#detail-growth').textContent=s.growth;document.querySelector('#chart-title').textContent=`${s.name} 估值历史（TTM 估算）`;document.querySelector('#insight-copy').textContent=s.note;document.querySelector('#model-inputs').innerHTML=modelInputs(s);drawChart(s);window.scrollTo({top:0,behavior:'smooth'})}
...
Promise.all([fetch('data/stocks.json',{cache:'no-store'}),fetch('data/history.json',{cache:'no-store'}),fetch('data/nasdaq.json',{cache:'no-store'}),fetch('data/concentration.json',{cache:'no-store'})]).then(async([market,history,nasdaq,concentration])=>{if(market.ok){const data=await market.json();if(Array.isArray(data.stocks)&&data.stocks.length){const fetched=data.stocks.filter(stock=>!['SPCX','SKHY'].includes(stock.ticker));const missing=fallbackStocks.filter(stock=>!fetched.some(item=>item.ticker===stock.ticker));stocks=[...fetched,...missing];lastUpdated=data.updatedAt?.replace('T',' ').replace('Z',' UTC')||lastUpdated;renderStocks();const stamp=document.querySelector('.updated b');if(stamp)stamp.textContent=lastUpdated}}if(history.ok){const data=await history.json();historyByTicker=data.stocks||{}}if(nasdaq.ok){nasdaqData=await nasdaq.json();renderNasdaq()}if(concentration.ok){concentrationData=await concentration.json();renderMag7Share()}}).catch(()=>{});renderStocks();renderNasdaq();
...
// Alpha Vantage's overview exposes trailing and forward PE.  With the same
// share price, TTM PE / forward PE - 1 is the market-implied EPS change.
const refreshGrowthPanels=()=>{
  const detail=document.querySelector('#detail');
  if(detail.classList.contains('hidden'))return;
  const stock=stocks[activeIndex],pe=Number(stock.pe),forwardPe=Number(stock.fpe);
  const epsGrowth=Number.isFinite(pe)&&Number.isFinite(forwardPe)&&pe>0&&forwardPe>0
    ?(pe/forwardPe-1)*100:null;
  document.querySelector('#detail-growth').textContent=stock.growth||'—';
  document.querySelector('#detail-eps-growth').textContent=epsGrowth===null?'—':`${epsGrowth>=0?'+':''}${epsGrowth.toFixed(0)}%`;
};
...
const syncHistorySummaryCards=()=>{
  ...
  document.querySelectorAll('#stats .stat').forEach((card,index)=>{
    ...
    const label=['PE（TTM）','Price / CF（TTM）','Price / Sales（TTM）'][index];
    card.querySelector('.stat-label').lastChild.nodeValue=label;
    card.querySelector('.stat-value').innerHTML=`${value.toFixed(1)}<span class="stat-sub">x</span>`;
    const subs=card.querySelectorAll('.stat-sub');
    if(subs[1])subs[1].textContent=`历史最新：${latest.date}`;
  });
};
new MutationObserver(syncHistorySummaryCards).observe(document.querySelector('#stats'),{childList:true});

// The existing grid already reserves a column between P/S and EV/EBITDA.
// Fill it with P/CF after every table render without fabricating missing data.
const insertPcfColumn=()=>document.querySelectorAll('#stock-rows .stock-row').forEach(row=>{
  if(row.querySelector('.pcf-home'))return;
  const stock=stocks.find(item=>item.ticker===row.dataset.ticker),cells=row.querySelectorAll('.cell');
  if(!stock)return;
  const cell=document.createElement('div');
  cell.className=`cell pcf-home ${rankClass(stock,'pcf')}`;
  cell.textContent=stock.pcf||'—';
  row.insertBefore(cell,cells[5]||row.querySelector('.implied'));
});
new MutationObserver(insertPcfColumn).observe(document.querySelector('#stock-rows'),{childList:true});
insertPcfColumn();
...
const renderMobileStocks=()=>{
  ...
  target.innerHTML=ordered.map(stock=>{
    ...
    return `<article class="mobile-stock-card" data-ticker="${stock.ticker}">
      ...
      <div class="mobile-more-grid"><span><small>PEG <button class="mobile-metric-help" data-metric="peg" aria-label="PEG说明">?</button></small><b>${multiple(stock.peg)}</b></span><span><small>市销率 <button class="mobile-metric-help" data-metric="ps" aria-label="市销率说明">?</button></small><b>${multiple(stock.ps)}</b></span><span><small>市现率 <button class="mobile-metric-help" data-metric="pcf" aria-label="市现率说明">?</button></small><b>${multiple(stock.pcf)}</b></span><span><small>EV/EBITDA <button class="mobile-metric-help" data-metric="ev-ebitda" aria-label="EV EBITDA说明">?</button></small><b>${multiple(stock.evEbitda)}</b></span><span><small>隐含增长率 <button class="mobile-metric-help" data-metric="implied-growth" aria-label="隐含增长率说明">?</button></small><b>${stock.implied||'—'}</b></span><span><small>收入同比 <button class="mobile-metric-help" data-metric="revenue-growth" aria-label="收入同比说明">?</button></small><b>${stock.growth||'—'}</b></span></div>
    </article>`;
  }).join('');
...
document.addEventListener('mousemove',event=>{
  ...
  const seriesValues=isPrice?points.map(item=>Number(item.price)).filter(Number.isFinite):points.flatMap(item=>['pe','pcf','ps'].map(key=>Number(item[key])).filter(Number.isFinite));
  const low=Math.min(...seriesValues),high=Math.max(...seriesValues),padding=isPrice?Math.max((high-low)*.1,1):0;
  const min=isPrice?Math.max(0,low-padding):Math.max(0,Math.floor(low/5)*5),max=isPrice?high+padding:Math.ceil(high/5)*5||5;
  ...
  const rows=(isPrice?[['price','日收盘价','#1ba6a0']]:[['pe','PE（TTM）','#ee8b2d'],['pcf','Price / CF（TTM）','#1ba6a0'],['ps','Price / Sales（TTM）','#b547c3']])
    .filter(([key])=>numericValue(point[key]))
    .map(([key,label,color])=>`<div style="display:grid;grid-template-columns:10px 1fr auto;align-items:center;gap:7px;margin-top:7px"><i style="display:block;width:8px;height:8px;border-radius:50%;background:${color}"></i><span style="font-size:12px;color:#52615a">${label}</span><strong style="font:700 14px 'DM Mono',monospace;color:#16231d">${isPrice?'$':''}${Number(point[key]).toFixed(isPrice?2:1)}${isPrice?'':'x'}</strong></div>`).join('');
