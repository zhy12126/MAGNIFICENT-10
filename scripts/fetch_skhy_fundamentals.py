...
TERMINAL_GROWTH = .025
TTM_WEIGHT = .35
# FY2021 consolidated K-IFRS figures, KRW million. Yahoo currently exposes only
# four annual columns for 000660.KS, so retain the next official year needed for
# a full-cycle median. Source: Korean DART annual report 20220322000590.
OFFICIAL_ANNUAL_FCF_INPUTS = {
    2021: {"revenue": 42_997_792, "operatingCashflow": 19_797_648, "capitalExpenditure": 12_486_635},
}
...
def main():
    ...
    for col in sorted(set(annual_income.columns) & set(annual_cash.columns), reverse=True):
        ...
        if annual_revenue and annual_cfo is not None and annual_capex is not None:
            ...
            observed_years.add(col.year)
    for year, row in sorted(OFFICIAL_ANNUAL_FCF_INPUTS.items(), reverse=True):
        if year not in observed_years:
            margins.append((row["operatingCashflow"] - row["capitalExpenditure"]) / row["revenue"])
    if len(margins) < 3:
    ...
    model = {
        ...
        "source": ttm["source"] + "; FY2021 cycle input from Korean DART annual report; KRW translated at latest KRW/USD",
